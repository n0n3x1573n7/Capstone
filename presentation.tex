...
\begin{document}
	...
	\begin{frame}{Quantumizing S-DES}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\item Currently working on breaking it down to fundamental and/or/not-gates
				\begin{itemize}
					\item Possibly change up the set of gates?
					\item Possibly automate generation on S-Boxes on actual DES?
				\end{itemize}
			...
			\begin{itemize}
				...
				\item Currently looking for a workaround; i.e. partially using classic bits or use 0-initialized qubits and XOR operation.
				\item Increasing the number of qubits(i.e. using Ancilla Bits) is a final resort.
			\end{itemize}
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item Broken S-DES down to its fundamentals; working on conversion to quantum.
			\item Implement a non-quantum S-DES for testing the quantum version(once completed).
		\end{itemize}
	\end{frame}
