...
\begin{document}
	...
	\begin{frame}{Quantum S-DES Implementation}
		\begin{itemize}
			\item S-DES implementation done except S-Box
            \begin{itemize}
                \item Not validated yet
                \item Other parts were easily done with the aid of Microsoft Q\# Library
            \end{itemize}
            \item By introducing ancilla bits (which are only used within each round), detouring No-cloning theorem was possible
            \item Applying Grover's Algorithm would not be a problem (except for speed)
            \item Took 20 seconds of running time for a single iteration of S-DES, which was slower than expected
            \begin{itemize}
                \item Constructing efficient circuit would be grateful
            \end{itemize}
		\end{itemize}
	\end{frame}
